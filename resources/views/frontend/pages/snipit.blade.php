...
<head>
    ...
    <title>{{ $snipit->meta_title }}</title>
    <meta name="description" content="{{ $snipit->meta_description }}">
    <meta name="keywords" content="{{ $snipit->meta_keyword }}">
    <link rel="canonical" href="{{ url($snipit->canonical_tag) }}" />

    <!--- OG meta tags start -->
    <meta property="og:title" content="{{ $snipit->meta_title }}" />
    <meta property="og:description" content="{{ $snipit->meta_description }}" />
    <meta property="og:type" content="article" />
    <meta property="og:image" content="{{asset($snipit->hero_image)}}" />
    <meta property="og:url" content="{{ url($snipit->canonical_tag) }}" />
    <!--- OG meta tags end -->
    ...
    <script type="application/ld+json">
        {
          ...
          "mainEntityOfPage": {
            "@type": "WebPage",
            "@id": "{{ url($snipit->canonical_tag) }}"
          },
          "headline": "{{ $snipit->meta_title }}",
          "description": "{{ $snipit->meta_description }}",
          "image": "{{asset($snipit->hero_image)}}",
          "author": {
            "@type": "Person",
            "name": "ResumeSathi"
          },

          "datePublished": "{{ $snipit->created_at->format('Y-m-d\TH:i:sP') }}"
        }
    </script>
{{-- Schema start --}}
...
<body style="background-color: #282c34;">
    <div class="play-ground-mob-tab">
        <div>
            <img loading="lazy" src="{{ asset('front-assets/images/icons/playground-html.svg') }}" width="16" height="16" alt="HTML Code">
            HTML
        </div>
        <div>
            <img loading="lazy" src="{{ asset('front-assets/images/icons/playground-css.svg')}}" width="16" height="16" alt="CSS Code">
            CSS
        </div>
        <div>
    ...
    <div class="code-playground" id="playground">
        <div class="editors d-none d-lg-block">
            <div class="editor-pane mob-tab" id="html-pane">
                ...
                <textarea class="editor-content" id="html-content"> {{ $snipit->codes->html }}</textarea>
            </div>

            <div class="editor-pane mob-tab" id="css-pane">
                <div class="editor-header">
                    <img loading="lazy" src="{{ asset('front-assets/images/icons/playground-css.svg') }}" width="16" height="16" alt="HTML Code">
                    CSS
                </div>
                <textarea class="editor-content" id="css-content">{{ $snipit->codes->css }}</textarea>
            </div>
            ...
            <div class="editor-pane mob-tab" id="js-pane">
                ...
                <textarea class="editor-content" id="js-content">{{ $snipit->codes->javascript }}</textarea>
            </div>
        </div>

        <div class="output mob-tab" id="output">
            <h1 class="editor-header output-title">{{ $snipit->title }}</h1>
    ...
    <script>
        ...
        function updateOutput() {
            ...
            outputDoc.open();
            outputDoc.write(htmlEditor.getValue() + '<style>' + cssEditor.getValue() + '</style>' + '<script>' + jsEditor.getValue() + '<\/script>');
            outputDoc.close();
        }
